...
df <- f_counts %>%
  ...
  select(-filename, -plant) %>% # look at non-plant foods
  ...
  mutate(total_counts = algae + animal + fungi + mineral + tomato_seedling) %>%
  mutate_at(vars(algae, animal, fungi, mineral, tomato_seedling), list(~(. / total_counts)*100)) %>% # get proportions of each food group
  pivot_longer(c("algae", "animal", "fungi", "mineral", "tomato_seedling"), 
...
p2 <- ggbarplot(df, x = "Ratio_Tomato", y = "relative_abundance", fill = "food_group", width = 0.5, lwd = 0.2,
                ...
                xlab = "Proportion of biospecimen:seedling",
                palette = c("dodgerblue", "wheat", "thistle", "chocolate", "palegreen3"))
